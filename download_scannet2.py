...
import tempfile
# from imp import reload # 在 Python 3.4 及之后的版本中，imp模块已被弃用，不建议再使用。
# reload(sys)
import importlib # 用这个代替 imp
...
def get_release_scans(release_file):
    scan_lines = urllib.request.urlopen(release_file)  # python3.x用这一行
    # scan_lines = urllib.urlopen(release_file)  # python2.7用这一行
    scans = []
